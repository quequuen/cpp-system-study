...
- 객체 자체는 스택에 있을 수 있지만 실제 데이터는 힙에 저장. push_back 이후 변수 자체의 메모리 주소는 변하지 않지만 실제 데이터의 주소는 변함.
...
| 구분            | 일반 배열 (`int arr[]`)       | 벡터 (`std::vector<int> v`)      |
| :-------------- | :---------------------------- | :------------------------------- |
| **데이터 위치** | 스택 (Stack)                  | 힙 (Heap)                        |
| **변수 주소**   | `&arr == &arr[0]` (동일 개념) | `&v != &v[0]` (완전히 다른 개념) |
| **유연성**      | 크기 고정, 이사 불가          | 크기 가변, 이사 가능             |
| **이사 시**     | 해당 안됨                     | 내부 주소값만 갱신, `&v`는 고정  |

참고: [stl.md#vector](./stl.md#stdvector)
