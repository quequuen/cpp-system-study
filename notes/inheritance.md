...
- 하이드 (Hide)된 부모 멤버를 쓰는 방법
  - 범위 지정 연산자(::) 사용
    ...
    - 매번 부모 클래스의 것을 쓰겠다고 명시하는 방법
    - 자식 클래스에서 이름이 같은 함수를 하나라도 선언하면, 부모의 다른 모든 오버로딩 버전(매개변수가 다른 것들)이 먹통이 됨.
    - 부모 클래스의 이름이 바뀌었을 때, 전부 찾아서 수정해야 함.
  ...
  - `using` 선언
    ...
    ```
    코드의 의도를 명확히 하고, 실수를 방지하기 위해서 더 권장됨.
    - 부모의 함수들이 자식 클래스의 멤버인 것처럼 인터페이스에 합쳐짐.
    - 부모의 다양한 버전들과 자식의 새로운 버전들이 한꺼번에 오버로딩 후보군에 올라감. 호출 시에 컴파일러가 알아서 가장 적절한 것을 찾아줌.
    - 유지보수가 간편함.
    - 접근 제어(`public`, `protected` 등)를 유연하게 할 수 있음.
...
- 기능을 재정의해서 다형성을 활용하는 목적이라면 당연히 오버라이딩(Overriding)이 더 간편하고 올바른 방법이지만 **의도적으로 하이딩(Hiding)을 사용하는 경우**가 있음.
  1. 부모의 특정 기능을 숨기거나 교체하고 싶을 때
     부모 클래스로부터 상속을 받았지만, 자식 클래스에서는 부모의 특정 인터페이스를 사용자가 쓰지 못하게 막거나 완전히 다른 용도로 대체하고 싶을 때 사용.
  2. 이름 충돌 해결 및 명확한 인터페이스 제공
     여러 라이브러리를 가져다 쓸 때 부모 클래스에 원치 않는 이름의 함수가 많을 수 있음. 이때 자식 클래스에서 같은 이름의 함수를 선언하여 부모의 복잡한 함수들을 가려버리고, 자식이 제공하는 함수만 보이게 할 수 있음.
  3. 템플릿 프로그래밍에서의 이름 탐색 제어
     매우 복잡한 템플릿 클래스 상속 구조에서는 컴파일러가 이름을 찾는 순서를 제어하기 위해 하이딩을 발생시키고, 필요한 것만 `using`으로 끌러올려 쓰기도 함.
  - C++은 이름이 같으면 해당 스코프에서 가장 먼저 발견된 것을 우선한다는 엄격한 규칙을 가지기 때문에 하이딩이 존재함. (인터페이스의 단절을 의미함)
  - 현업에서는 부모의 기능을 확장할 때는 오버라이딩을 쓰고, 부모로부터 물려받은 이름이 자식 클래스의 역할과 맞지 않아 가려야 할 때만 하이딩을 발생시킴. 그리고 그 과정에서 발생하는 불편함(오버로딩 깨짐 등)을 해결하기 위해 `using` 선언이라는 보완책을 씀.
