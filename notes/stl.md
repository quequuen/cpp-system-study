...
반복자는 컨테이너(데이터 저장소)와 알고리즘(데이터 처리) 사이를 이어주는 가교. stl마다 정렬 함수를 각각 따로 만들어야 하는 상황을 반복자 하나로 편리하게 해줌.

- 반복자는 **포인터처럼 보이게 설계된 객체**임. 그래서 포인터와 똑닽은 연산자를 씀.

- 반복자 5가지
  컨테이너의 특성에 따라 사용할 수 있는 능력이 제한됨.
  - 입력 반복자 (Input Iterator): 읽기만 가능, 한 방향으로만 전진 (예: 키보드 입력 스트림).
  - 출력 반복자 (Output Iterator): 쓰기만 가능, 한 방향으로만 전진 (예: 모니터 출력 스트림).
  - 순방향 반복자 (Forward Iterator): 읽기/쓰기 가능, 한 방향 전진 (예: `forward_list`).
  - 양방향 반복자 (Bidirectional Iterator): `++`와 `--` 가능 (예: `list`, `map`, `set`).
  - 임의 접근 반복자 (Random Access Iterator): `+`, `-`, `[]` 가능 (예: `vector`, `deque`, `array`).

- 반개방 범위 (Half-open range)
  STL에서 범위는 항상 `[begin, end)` 형태.
  - `begin()`: 첫 번째 원소를 가리킴.
  - `end()`: 마지막 원소가 아닌 **마지막 원소의 다음(빈 공간)**을 가리킴.
...
집합. 값 자체가 키가 되는 구조. 수학에서의 집합처럼 중복을 허용하지 않고 데이터를 관리할 때 사용.

- **중복 불가**: 똑같은 값을 두 번 넣어도 하나만 저장.
- **자동 정렬**: 데이터를 넣기만 하면 내부적으로 오름차순 정렬을 유지함. (내부 구조가 `map`과 같은 Red-Black Tree)
- **수정 불가**: 한 번 들어간 값은 수정이 불가함. 값을 바꾸고 싶다면 기존 값을 지우고 새 값을 넣어야 합니다. (정렬 순서가 망가지는 걸 방지하기 위함임)
...
- 값 찾기에서 end()는 실제 데이터가 들어있는 마지막 칸이 아닌, 가상의 빈칸을 가리키는 주소값.
- 개수 확인에서 count()를 사용하는 이유는 set은 자료가 중복되지 않기 때문에 count가 곧 존재/비존재가 될 수 있음.
...
1. 원본을 수정하지 않는 알고리즘 (Non-modifying)
   데이터를 변경하지 않고 관찰하거나 위치만 찾아내는 알고리즘.

- `std::find`/`std::find_if`: 값을 찾거나, 조건에 맞는 위치를 찾음.
- `std::count`/`std::count_if`: 개수를 셈.
- `std::all_of`, `std::any_of`: 전부, 하나라도~와 같은 질문에 결과를 반환.
  - 처음부터 끝까지 한 칸씩 이동하며 확인하므로 시간 복잡도 $O(N)$.

2. 원본을 수정하는 알고리즘 (Modifying)
   데이터의 값을 바꾸거나 위치를 옮기는 알고리즘.

- `std::copy`: 데이터를 다른 곳으로 복사함.
- `std::replace`: 특정 값을 다른 값으로 일괄 교체함.
- `std::remove`: 데이터를 실제로 지우는 게 아닌 지울 데이터를 뒤로 밀어버림. (추후 `vector::erase`와 함께 쓰는 Erase-Remove 패턴이 필수)

3. 정렬 및 관련 알고리즘 (Sorting & Relational)

- `std::sort`: 기본적으로 Introsort 기번을 사용. (퀵 정렬로 시작해서 깊어지면 힙 정렬, 데이터가 적으면 삽입 정렬로 자동 전환되는 하이브리드 방식)
- `std::stable_sort`: 같은 값들의 원래 순서를 보장하며 정렬함.
- `std::binary_search`: 반드시 정렬된 데이터에서만 사용. 대신 빠름. ($O(\log N)$)

4. 이분 탐색 및 범위 관련 (Binary Search & Bounds)
   정렬된 데이터에서 경계값을 찾을 때 사용되는 알고리즘.

- `std::lower_bound`: 찾으려는 값 이상이 처음 나오는 위치 반환.
- `std::upper_bound`: 찾으려는 값을 초과하는 값이 처음 나오는 위치 반환.
  - 두 함수의 차이를 이용하면 정렬된 배열에서 찾으려는 값이 총 몇 번 나오는지 $O(\log N)$ 만에 계산 가능.
