...
크기가 자유롭게 변하는 동적 배열. 일반 배열은 처음에 정한 크기를 바꿀 수 없지만, vector는 데이터가 늘어나면 스스로 메모리를 확장함
...
- 들어오는 데이터의 크기를 미리 알 수 있는 경우, `reserve()`를 사용하여 미리 공간을 예약한다면 Reallocation 횟수를 줄여 성을을 획기적으로 높일 수 있음.
