...
- 내부 동작
  ...
  - **동적 할당(New)**: 다양한 크기의 람다를 가리키기 위해 `new Holder`를 통해 메모리를 힙 영역에 할당함.
  - **간접 참조(가상 함수 테이블 탐색)**: 함수를 직접 호출하지 못하고, 포인터를 타고 들어가 가상 함수 테이블(Vtable)을 거쳐 진짜 함수 주소를 찾아내야 함. 이 때문에 컴파일러는 인라인(Inline) 최적화가 불가능해짐.
