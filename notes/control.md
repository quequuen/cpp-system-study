...
두 값을 비교하여 결과값으로 참(true) 또는 거짓(false)을 반환하는 연산자. 제어 흐름의 if문이나 while문에서 조건을 판단할 때 핵심적으로 사용됨.
...
- 우주선 연산자(세 방향 비교 연산자, Three-way comparison operator, <=>)

  - (a <=> b);
  - 한 번의 연산으로 두 값이 '작은지, 같은지, 큰지'를 모두 알려줌.
  - a < b 이면 음수 값 반환, a == b이면 0 반환, a > b 이면 양수 값 반환.

- double이나 float 같은 실수 타입은 정밀도 문제 때문에 == 연산자로 비교하면 위험(epsilon을 비교해서 측정).
- 기본적으로 사용자가 만든 구조체에서는 ==로 직접 비교할 수 없음. 연산자 오버로딩을 하거나 멤버 변수별로 비교해야 함.
