...
1. 데이터 발생: 키보드로 `Hello` 입력.
2. 스트림에 탑승: 데이터가 입력 스트림(cin)이라는 통로에 들어감.
3. 버퍼에 대기: 스트림을 따라 가다가 프로그램에 바로 전달되지 않고, 일단 입력 버퍼라는 임시 창고에 쌓임. (보통 엔터 키를 누를 때까지 기다림)
4. 프로그램 전달: 엔터를 누르는 순간, 버퍼에 쌓여있던 데이터가 프로그램으로 한꺼번에 쏟아져 들어옴.
