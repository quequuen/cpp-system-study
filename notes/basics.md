...
- 파일 읽기 (Loading)
  ...
  - `exit(1)`보다 `return 1;`을 쓰는 이유
    ...
    - `exit(1)`의 동작
      `exit()` 함수는 호출되는 즉시 프로그램을 강제로 종료시키는 C 언어 스타일의 함수.
      - **소멸자 생략**: `exit()`이 호출되면 **현재 스택에 쌓여있는 지역 객체들의 소멸자가 호출되지 않음.**
      - **정적 객체만 정리**: 오직 전역 객체(Global)나 `static` 객체들의 소멸자만 호출될 뿐, `main` 함수나 다른 함수 내부에 선언된 일반적인 객체들은 정리되지 못하고 프로세스가 종료됨.
