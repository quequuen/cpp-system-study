...
C++에는 interface라는 키워드가 따로 없지만, 모든 함수가 순수 가상 함수로만 이루어진 클래스를 관례적으로 인터페이스라고 부름. **(계약서)**
...
| --------- | -------------------------------------- | ------------------------------------- |
| 구성 요소 | 순수 가상 함수 + 일반 함수 + 멤버 변수 | 오직 순수 가상 함수만 존재            |
| 목적      | 상속 관계의 공통 기능 확장 (Is-A)      | 객체 간의 상호작용 규격 정의 (Can-Do) |
| 상속      | 주로 단일 상속 (계층 구조)             | 주로 다중 상속 (기능 조립)            |
| 비유      | **설계도**: 기본 뼈대를 구축           | **계약서**: 최소 규약을 구축          |
