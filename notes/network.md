...
- Byte Stream
  - 메시지 단위(패킷 단위)가 아닌 연속된 **Byte Stream**으로 데이터 전달. 예를 들어 클라이언트가 `"Hello"`라는 데이터를 한 번에 보냈다고 해서 서버가 반드시 `read()` → `"Hello"`를 한 번에 받는다고 보장되지 않음. `read()` → `"Hel"`, `read()` → `"llo"`와 같이 송신 측의 `send()` 호출 단위와 수신 측의 `recv()` 호출 단위가 일치한다는 보장이 없음.
  - `Message Boundary`를 보장하지 않음. → Chat Server를 만들 때 굉장히 중요함. `Message Framing 정의`의 이유가 되기 때문.
    - TCP는
...
- `read()` / `recv()`
  `accept()`를 통해 연결된 Client에게 실제 데이터를 전달하는 단계. `read()`와 `recv()`는 둘 다 데이터를 읽지만, `recv()`는 소켓 통신을 위해 추가적인 옵션을 제공하는 함수.
...
`Read error: Bad file descriptor...` 발생
해당 코드는 Session이 지역 변수이기 때문에 detach() 이후에 죽어버림.
detach()로 인해 Thread는 살아있지만 세션은 먼저 죽었기 때문에 연결 유지 불가능. 그래서 shared_ptr이 필요.

- `shared_ptr` Session 생성.
...
make_shared()로 인해 main의 지역변수 session이 사라져도 Thread의 작업을 끝낼 때까지 Session이 살아있을 수 있음.
위처럼 &session을 하게 되면 shared_ptr의 주소(포인터)가 되어버리기 때문에 &를 떼어야 함. Session::run()은 Session 객체에서 실행되어야 하기 때문 &session이 아닌 Session 객체 자체를 원하는 것.
...
그렇기 때문에 `vector`를 사용. 각 Session을 `vector`에 저장.
...
`disconnect` 시 `sessions`이란 객체 내에서 자기 자신을 찾은 후, 제거하고 소켓을 닫음.
...
- `run()`
  ...
  현재 `run()` 내부에서 Client에게서 받은 데이터를 `receive_buffer`에 누적한 후, `\n` 기준으로 framing을 한 후 `broadcast()`에 넘김.
