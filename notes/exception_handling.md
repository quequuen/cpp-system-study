...
프로그램 실해 중 발생하는 예기치 못한 상황(에러)을 감지하고 제어하는 메커니즘. 에러 처리 코드와 일반 로직을 분리하여 가독성을 향상시키지만 C++ 같은 경우, 생성자나 소멸자에서 예외가 발생할 때의 리스크(메모리 누수 등)을 반드시 고려해야 함. 또한, 반복문 내에서의 예외 처리는 퍼포먼스가 떨어지기 때문에 이 부분도 고려해야 함. 보통 사용자 입력값 유효성 검사 이런 부분들은 사용자가 올바르게 입력할 수 있게 유도하는 방향으로 이끌고 네트워크/서버 쪽에서 예외 처리를 많이 사용.
...
- catch 블록의 매개변수 타입과 예외 처리에서 throw된 예외 객체의 타입은 **엄격하게 일치**해야 함.
...
- 이 원칙과 기능 덕분에 C++에는 `finally` 기능이 없음. 이미 `RAII` 방식으로 표준 라이브러리인 스마트 포인터(`std::unique_ptr`, `std::shared_ptr`)나 자물쇠(`std::scoped_lock`)등이 이미 잘 구현이 되어 있기 때문. 하지만 파일이나 메모리가 아니라, "함수가 끝날 때 특정 전역 플래그 변수를 무조건 false로 되돌려야 한다" 같은 일반적인 로직의 `finally`가 필요할 때는 임의로 **커스텀 가드(Gaurd) 객체**를 만들어서 해결함.
