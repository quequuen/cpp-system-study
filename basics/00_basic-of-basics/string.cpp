...
int main (){
    ...
    // assign을 사용하는 이유는 이런 식으로 정밀하게 특정 범위만 골라서 대입하거나
    // 아래처럼 초기화와 동시에 여러번 반복을 하고 싶을 때 사용
    // 그리고 가장 큰 이유는 '데이터 재사용' 때문임.
    // s10 = "New" // 는 경우에 따라 기존 메모리를 버리고 새로 할당할 수도 있음.
    // s10.assign("New") // 은 기존에 가지고 있는 메모리는 그대로 두고, 내용물만 바꾼 후 'New'로 채우라는 의미가 강함.
    s10.assign(200, 'A');     // "AAAAA"
    // SSO 때문에 너무 적은 데이터로는 스왑 안됨!

    std::cout << "s9: " << s9 << std::endl;
    std::cout << "s9 주소: " << (void*)s9.c_str() << std::endl;
    // c++은 s9.c_str() 이렇게만 하면 주소확인이 아닌 해당 주소에 담긴 값을
    // 알아서 읽어주기 때문에 그걸 방지하기 위해 void*를 해주어야 함.
    std::cout << "s10: " << s10 << std::endl;
